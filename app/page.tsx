  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <section className="mb-16 text-center">
        ...
        <form onSubmit={handleSearch} className="mx-auto mt-8 flex max-w-md items-center gap-3 rounded-2xl border-2 border-slate-950 bg-white px-5 py-4 shadow-lg transition hover:border-slate-950 hover:shadow-xl dark:border-slate-200 dark:bg-slate-900 dark:hover:border-slate-200">
          <Search className="h-5 w-5 flex-shrink-0 text-slate-700 dark:text-slate-300" />
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="搜索文章、笔记、论文..."
            className="flex-1 bg-transparent text-slate-900 outline-none placeholder:text-slate-500 dark:text-slate-100 dark:placeholder:text-slate-400"
          />
          <button type="submit" className="rounded-lg bg-slate-950 px-5 py-2 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-950 dark:hover:bg-slate-200">
            搜索
          </button>
        </form>
      </section>
